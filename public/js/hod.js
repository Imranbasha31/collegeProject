...
document.addEventListener('DOMContentLoaded', async () => {
  user = API.requireAuth(['hod']);
  if (!user) return;
  document.getElementById('userName').textContent = user.name;
  document.getElementById('avatarInitial').textContent = user.name[0].toUpperCase();
  document.getElementById('deptInfo').textContent = `Department: ${user.department_name || '—'}`;
...
async function loadLeaves() {
  try {
    ...
    const pending = allLeaves.filter(l => l.status === 'pending_hod');
...
function updateKPIs(leaves) {
  ...
  document.getElementById('kpi-approved').textContent = leaves.filter(l => ['pending_principal','approved'].includes(l.status)).length;
...
function renderPending(leaves) {
  ...
  if (!leaves.length) {
    tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="empty-icon">✅</div><h3>No pending requests</h3><p>All advisor-approved requests have been reviewed</p></div></td></tr>`;
    return;
  }
  tbody.innerHTML = leaves.map(l => `
    <tr>
      <td><div style="font-weight:600">${l.student_name}</div></td>
      <td style="color:var(--text-muted);font-size:0.8rem">${l.class || '—'}</td>
      ...
      <td style="color:var(--text-muted);font-size:0.8rem">Advisor Approved</td>
      <td>
...
function renderAll(leaves) {
  ...
  if (!leaves.length) {
    tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="empty-icon">📋</div><h3>No leave records</h3></div></td></tr>`;
    return;
  }
  tbody.innerHTML = leaves.map(l => `
    <tr>
...
async function openAction(leaveId) {
  ...
  try {
    const { leave } = await API.get(`/leaves/${leaveId}`);
    document.getElementById('actionModalContent').innerHTML = `
      ...
      <div class="form-group">
        <label class="form-label">HOD Comment (optional)</label>
        <textarea class="form-control" id="actionComment" rows="3" placeholder="Add your decision note..."></textarea>
      </div>`;
...
async function submitAction(action) {
  ...
  try {
    ...
    API.showToast(action === 'approved' ? 'Leave approved and forwarded to Principal!' : 'Leave rejected.', action === 'approved' ? 'success' : 'danger');
    await loadLeaves();
  } catch(e) {
  ...
  try {
    ...
    document.getElementById('detailContent').innerHTML = `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px">
        ...
        <div><div style="font-size:0.72rem;color:var(--text-muted)">Department</div><div style="font-weight:600">${leave.department_name||'—'}</div></div>
